...
var defaultGranularities = map[string]Granularity{
	...
	"1min": Granularity{"1min", 7 * Day, Minute, 7 * Hour},
...
func (t *TimeSeries) key(granularity Granularity, timestampInSeconds int64) string {
	roundedTimestamp := t.roundTimestamp(granularity, timestampInSeconds)
	return fmt.Sprintf("%s:%s:%d", t.namespace, granularity.Name, roundedTimestamp)
}

func (t *TimeSeries) roundTimestamp(granularity Granularity, timestampInSeconds int64) int64 {
	return timestampInSeconds - (timestampInSeconds % granularity.Quantity)
}
...
func (t *TimeSeries) Insert(timestampInSeconds int64) {
	for _, granularity := range t.granularities {
		...
		field := strconv.FormatInt(t.roundTimestamp(granularity, timestampInSeconds), 10)
...
func (t *TimeSeries) Fetch(granularityName string, startTimestamp, endTimestamp int64) ([]Series, error) {
	...
	start := t.roundTimestamp(granularity, startTimestamp)
	end := t.roundTimestamp(granularity, endTimestamp)
	multi := t.client.Pipeline()

	var result []*redis.StringCmd
	for ts := start; ts <= end; ts += granularity.Duration {
		key := t.key(granularity, ts)
		field := strconv.FormatInt(t.roundTimestamp(granularity, ts), 10)
